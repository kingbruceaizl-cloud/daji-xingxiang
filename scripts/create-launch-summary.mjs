...
const githubHandoffFile = "daji-xingxiang-github-handoff.md";
const launchRunbookFile = "daji-xingxiang-launch-runbook.md";
...
const vercelEnvTemplateFile = "daji-xingxiang-vercel-env-template.env";
...
const lines = [
  ...
  `- Vercel 环境变量模板：${vercelEnvTemplateFile}（${fileStatus(vercelEnvTemplateFile)}）`,
  ...
  "## 自动化能力",
  "",
  "- `.github/workflows/verify.yml`：推送或拉取请求时运行本地发布验证、生产冒烟和线上素材连通性检查。",
  "- `.github/workflows/release-package.yml`：手动生成源码交付包，生成前会运行本地发布验证、生产冒烟和线上素材连通性检查，然后上传校验文件、发布清单、环境变量交接单、GitHub 仓库交接单、Supabase 初始化 SQL、Supabase 验收 SQL、Vercel 环境变量模板、Vercel 部署交接单、AI 模型通道交接单、上线执行核对单和上线摘要。",
